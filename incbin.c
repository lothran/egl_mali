...
static FILE *open_file(const char *name, const char *mode,
                       const char (*searches)[PATH_MAX], int count) {
  int i;
  for (i = 0; i < count; i++) {
...
#define s(IDENT) styled(style, IDENT)
  ...
  usage:
    fprintf(
        stderr,
        "%s [-help] [-Ipath...] | <sourcefiles> | [-o output] | [-p prefix]\n",
        argv[-1]);
    fprintf(stderr, "   -o         - output file [default is \"data.c\"]\n");
    fprintf(stderr, "   -p         - specify a prefix for symbol names "
                    "(default is \"g\")\n");
    fprintf(stderr, "   -S<style>  - specify a style for symbol generation "
  ...
  for (i = 0; i < files; i++) {
    ...
    while (fline(&line, &size, fp) != -1) {
      ...
      *--end = '\0';
      ...
      fprintf(out, "\n};\n");
      fprintf(out,
              "INCBIN_CONST INCBIN_ALIGN unsigned char *const %s%s%s = %s%s%s "
              "+ sizeof(%s%s%s);\n",
              prefix, name, s(kEnd), prefix, name, s(kData), prefix, name,
              s(kData));
